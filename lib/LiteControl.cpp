// This file is a copy of "Control.h" with "Player" class removed
...
bool LiteControl::set_input_filename(string filename,FILE_TYPE intype){
	InputS *ai=NULL;
...
string LiteControl::get_input_filename(){
	return wavinfo.filename;
};
string LiteControl::get_input_filename_and_info(){
...
/*string LiteControl::get_recommanded_output_filename(){
  return "none";
  };
  */


std::string LiteControl::get_stretch_info(){
	const int size=200;
	char tmp[size];tmp[size-1]=0;
...
string LiteControl::get_fftsize_info(){
	const int size=200;
	char tmp[size];tmp[size-1]=0;
...
string LiteControl::get_fftresolution_info(){
...
//void LiteControl::startplay(bool bypass){
...
////#endif
//};
//void LiteControl::pauseplay(){
//    player->pause();
//};
//
//void LiteControl::freezeplay(){
//    player->freeze();
//};

//void LiteControl::set_volume(REALTYPE vol){
//    volume=vol;
//    player->set_volume(vol);
//};
//
//
//void LiteControl::set_seek_pos(REALTYPE x){
//    seek_pos=x;
//    player->seek(x);
//};
//REALTYPE LiteControl::get_seek_pos(){
//    if (player->getmode()==Player::MODE_PLAY) seek_pos=player->info.position;
//    return seek_pos;
//};


void LiteControl::set_stretch_controls(double stretch_s,int mode,double fftsize_s,double onset_detection_sensitivity){
...
// NEW!
void LiteControl::set_stretch_direct_controls(double stretch, int bufsize, double onset_detection_sensitivity){
...
double LiteControl::get_stretch_control(double stretch,int mode){
	double result=1.0;
	switch(mode){
		case 0:
...
//void LiteControl::update_player_stretch(){
...
int LiteControl::get_optimized_updown(int n,bool up){
	int orig_n=n;
	while(true){
		n=orig_n;
#ifndef KISSFFT
...
int LiteControl::optimizebufsize(int n){
...
void LiteControl::set_window_type(FFTWindow window){
	window_type=window;
//    if (player) player->set_window_type(window);
};


string LiteControl::Render(string inaudio,string outaudio,FILE_TYPE outtype,FILE_TYPE intype,REALTYPE pos1,REALTYPE pos2){
	if (pos2<pos1){
		REALTYPE tmp=pos2;
		pos2=pos1;
		pos1=tmp;
...
string LiteControl::getfftsizestr(int fftsize){
	int size=100;
	char tmp[size];tmp[size-1]=0;
...
//void LiteControl::update_process_parameters(){
//    if (player) player->set_process_parameters(&ppar,&bbpar);
//};

bool LiteControl::save_parameters(const char *filename){
...
bool LiteControl::load_parameters(const char *filename){
	XMLwrapper *xml=new XMLwrapper();
